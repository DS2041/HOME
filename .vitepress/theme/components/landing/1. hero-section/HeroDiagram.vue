...
const inputFileSets = ref([
  // First pair (lines 1, 3, 5, 7)
  [
    { label: 'trading', color: '#ff8d67' },
    { label: '.ai', color: '#ff8d67' },
    { label: 'security' },
    { label: 'smart contract' }
  ],
  // Second pair (lines 1, 3, 5, 7)
  [
    { label: 'school' },
    { label: 'esports' },
    { label: 'web design', color: '#40b782' },
    { label: 'charity', color: '#40b782' }
  ]
])
...
// Add this with your other ref declarations at the top of the script
 ...
 */
const prepareInputs = () => {
  // Get the current file set (alternates between 0 and 1)
  const inputFileSet = inputFileSets.value[currentSetIndex.value % 2];
  currentSetIndex.value++; // Increment for next time

  // Use fixed line indexes 0, 2, 4, 6 (which correspond to lines 1, 3, 5, 7)
  const inputLineIndexes = [0, 2, 4, 6];
  ...
  inputLineIndexes.forEach((lineIndex, fileIndex) => {
    ...
    inputLines[lineIndex].value.dotColor = inputFileSet[fileIndex].color;
    inputLines[lineIndex].value.glowColor = inputFileSet[fileIndex].color;
  });
...
<template>
  <div class="hero__diagram" id="hero-diagram">
    ...
    <div class="vite-chip" :class="{ active: illuminateLogo }">
      ...
      <img :src="isUwu ? '/logo.svg' : '/logo.svg'" :alt="isUwu ? 'RAMI' : 'LOGO'" class="vite-chip__logo"
        :class="{ uwu: isUwu }" />
    </div>
  </div>
...
.hero__background {
  ...
  @media (min-width: 1024px) {
    background:
      url('/noise.png'),
      radial-gradient(circle at right center,
        rgba(63, 166, 123, 0.5) 0%,
        rgb(50, 119, 98) 25%,
        rgb(55, 140, 102) 40%,
        rgb(64, 168, 120) 65%,
        rgba(50, 115, 86, 0.5) 100%);
  ...
  @media (min-width: 1500px) {
    background:
      url('/noise.png'),
      radial-gradient(circle at right center,
        rgba(41, 105, 78, 0.5) 0%,
        rgb(50, 119, 87) 25%,
        rgb(55, 140, 105) 45%,
        rgb(64, 168, 135) 65%,
        rgba(50, 115, 91, 0.5) 100%);
